...
#
#   return the set of unique page links (in universal format --ie. no relative
#          links) on this url
#
...
from index.indexer import write_index # added Oct 2015 DJS
from index.indexer import make_index # added Oct 2015 DJS
...
def start_crawl(sys_argv):
    ...
    if (sys_argv[1] == "-w"):    #sart from a previous crawl
        process_wg_file(sys.stdin, links_already_dispatched, \
            hash_codes_already_visited, links_to_visit )
        url_matching_pattern = sys_argv[2]
        ###### if resuming index creation, need to add call here ######
    else:
        url_matching_pattern = sys_argv[1]
        starting_url = sys_argv[2]
        links_to_visit.add(starting_url)
    if (len(sys_argv) > 3):
        max_num_page_visited = int(sys.argv[3])
